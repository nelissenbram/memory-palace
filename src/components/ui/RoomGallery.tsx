        ...
        <div style={{ padding: "1.25rem 1.5rem 0", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", gap: "0.375rem", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", background: T.color.warmStone, borderRadius: "0.5rem", padding: "0.125rem" }}>
                <button onClick={() => setMode("grid")} aria-pressed={mode === "grid"} style={{
                  padding: "0.375rem 0.75rem", borderRadius: "0.375rem", border: "none", minHeight: "2.75rem",
          ...
          <div style={{ display: "flex", gap: "0.25rem", flexWrap: "wrap", marginBottom: "0.75rem" }}>
            <button onClick={() => setFilter(null)} aria-pressed={filter === null} style={{
              ...
              background: filter === null ? accent + "10" : T.color.white, minHeight: "2.75rem",
            ...
            {Object.entries(typeGroups).map(([type, items]) => (
              <button key={type} onClick={() => setFilter(type)} aria-pressed={filter === type} style={{
                ...
                background: filter === type ? accent + "10" : T.color.white, minHeight: "2.75rem",
                fontFamily: T.font.body, fontSize: "0.6875rem", color: filter === type ? accent : T.color.muted, cursor: "pointer",
              }}><TypeIcon type={type} size={13} color={filter === type ? accent : T.color.muted} /> {t(`type_${type}`)} ({items.length})</button>
            ))}
          </div>
        </div>
                    ...
                    <div key={type} style={{ background: T.color.white, borderRadius: "0.625rem", padding: "0.625rem 0.75rem", border: `1px solid ${T.color.cream}` }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.25rem" }}>
                        <span style={{ fontFamily: T.font.body, fontSize: "0.75rem", fontWeight: 500, color: T.color.charcoal }}>
                          <TypeIcon type={type} size={14} color={T.color.charcoal} /> {t(`type_${type}`)}
                        </span>
                      ...
                      {unallocated.map((mem) => {
                        ...
                        return (
                          <div key={mem.id} style={{
                            ...
                            {/* Thumb */}
                            <MediaThumb mem={mem} size={2.25} borderRadius="0.375rem" iconSize={18} iconColor={T.color.muted} />
                            {/* Info */}
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <div style={{ fontFamily: T.font.body, fontSize: "0.625rem", color: T.color.muted }}>
                                <TypeIcon type={mem.type} size={13} color={T.color.muted} /> {t(`type_${mem.type}`)} — {canDisplay ? `${limit - typeCount} ${limit - typeCount > 1 ? t("slotsAvailable") : t("slotAvailable")}` : t("allSlotsFull")}
                              </div>
                            </div>
                            {/* Display button */}
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem" }}>
                      {displayed.map((mem) => (
                        <button key={mem.id} onClick={() => toggleDisplay(mem)}
                          ...
                          <TypeIcon type={mem.type} size={13} color={T.color.charcoal} /> {mem.title.length > 20 ? mem.title.slice(0, 20) + "..." : mem.title}
                ...
                return (
                  ...
                  >
                    ...
                    <div role="button" tabIndex={0} onClick={() => openPlayer(idx)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openPlayer(idx); } }} style={{
                      height: "6.25rem", position: "relative", cursor: "pointer", overflow: "hidden",
                    }}>
                      <MediaThumb mem={mem} size="100%" borderRadius="0" iconSize={24} iconColor="rgba(255,255,255,0.4)" />
                    </div>
                    ...
                    <div style={{ padding: "0.5rem 0.625rem" }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "0.25rem" }}>
                        <span style={{ fontFamily: T.font.body, fontSize: "0.5625rem", color: T.color.muted, display: "inline-flex", alignItems: "center", gap: "0.1875rem" }}><TypeIcon type={mem.type} size={13} color={T.color.muted} /> {t(`type_${mem.type}`)}</span>
                        {/* Display toggle */}
                        <button
                          ...
                          style={{
                            width: isMobile ? "2.75rem" : "1.375rem", height: isMobile ? "2.75rem" : "1.375rem",
              ...
              <div style={{
                ...
                {playerMem.type === "video" || playerMem.videoBlob ? (
                  ...
                  <div style={{ padding: "3.75rem 2.5rem", textAlign: "center" }}>
                    <div style={{ marginBottom: "1rem" }}><TypeIcon type="audio" size={64} color="rgba(255,255,255,0.4)" /></div>
                  ...
                  <div style={{ padding: "3.75rem 2.5rem", textAlign: "center" }}>
                    <div style={{ marginBottom: "1rem" }}><TypeIcon type="document" size={64} color="rgba(255,255,255,0.4)" /></div>
                    <div style={{ fontFamily: T.font.display, fontSize: "1.25rem", color: T.color.cream, marginBottom: "0.5rem" }}>{playerMem.title}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "18.75rem", background: T.color.charcoal }}>
                    {playerMem.dataUrl ? (
                      ...
                      <div style={{
                        ...
                        <TypeIcon type={playerMem.type} size={48} color="rgba(255,255,255,0.4)" />
                      </div>
                    )}
                  </div>
                )}
              ...
              <div style={{ display: "flex", gap: "0.375rem", overflowX: "auto", paddingBottom: "0.25rem" }}>
                {filtered.map((mem, idx) => (
                  <button key={mem.id} onClick={() => { setPlayerIdx(idx); setPlaying(false); }}
                    ...
                    <MediaThumb mem={mem} size="100%" borderRadius="0" iconSize={18} iconColor="rgba(255,255,255,0.4)" />
                  </button>
                ))}
              </div>
            </div>
