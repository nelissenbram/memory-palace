...
export default function LandingPage({ initialIosApp = false }: { initialIosApp?: boolean }) {
  return (
    <LandingErrorBoundary fallback={<LandingErrorFallback />}>
      <LandingPageContent initialIosApp={initialIosApp} />
...
function LandingPageContent({ initialIosApp = false }: { initialIosApp?: boolean }) {
  ...
  // Seeded server-side from the mp_platform cookie / MemoryPalace-iOS UA so the
  // iOS build never renders paid CTAs ("— Free", upgrade copy) even in the raw
  // SSR HTML / first paint (Apple Guideline 3.1.1). The effect below re-confirms
  // client-side for the Capacitor runtime.
  const [isIosApp, setIsIosApp] = useState(initialIosApp);
  ...
  /* On iOS the app is 100% free — no in-app purchases, no trials, no paid tier.
     The "— Free" price qualifier on the CTAs is therefore meaningless there and,
     per Apple Guideline 2.1 review feedback, reads as if paid content exists.
     Strip the qualifier (everything after the em/en-dash) when in the iOS app.
     Works across all locales, which all use "<action> — <Free/Gratis/...>". */
  const iosCta = (label: string): string =>
    isIosApp && typeof label === "string"
      ...
      <main id="main-content">
        ...
        >
          ...
          >
            ...
            >
              {ctaLoading === "hero" ? (lAny?.loading) : iosCta(landing.hero.cta)}
            </button>
            <button
              onClick={() => {
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            >
              {ctaLoading === "bottom" ? (lAny?.loading) : iosCta(landing.cta.button)}
            </button>
          </div>
        </section>
      </main>
        ...
        >
          ...
          <div>
            ...
            >
              ...
              >
                {isIosApp ? landing.nav.getStarted : landing.footer.getStartedFree}
              </Link>
            </div>
          </div>
