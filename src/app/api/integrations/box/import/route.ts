...
export async function POST(request: NextRequest) {
  try {
    ...
    // Box import requires Keeper plan or higher
    const sub = await getUserPlan(user.id);
    if (sub.plan === "free") {
      return NextResponse.json({ error: "Box import requires a Keeper or Guardian plan" }, { status: 403 });
    }
    ...
    for (const fileId of fileIds) {
      ...
      try {
        ...
        // Box may return application/octet-stream — infer from extension
        ...
        if (data.byteLength > MAX_IMPORT_FILE_SIZE) {
          results.push({ id: fileId, success: false, error: `File too large (${Math.round(data.byteLength / 1024 / 1024)}MB). Maximum is ${MAX_IMPORT_FILE_SIZE / (1024 * 1024)}MB.` });
          continue;
        }
        ...
        if (isR2Configured()) {
          try {
            ...
            results.push({ id: fileId, success: false, error: "Upload failed" });
            continue;
          }
        } else {
...
/** Infer MIME type from file extension (Box may return application/octet-stream). */
