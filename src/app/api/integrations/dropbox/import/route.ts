...
export async function POST(request: NextRequest) {
  try {
    ...
    // Dropbox import requires Keeper plan or higher
    const sub = await getUserPlan(user.id);
    if (sub.plan === "free") {
      return NextResponse.json({ error: "Dropbox import requires a Keeper or Guardian plan" }, { status: 403 });
    }
      ...
      try {
        ...
        if (isR2Configured()) {
          try {
            ...
            results.push({ id: filePath, success: false, error: "Upload failed" });
            continue;
          }
        } else {
        ...
        if (memErr) {
          ...
          void detectRequestPlatform().then((platform) =>
            captureServer(user.id, "memory_created", { source: "import", provider: "dropbox", ...(platform ? { platform } : {}) })
          );
          void captureFirstMemoryMilestone(user.id, { source: "import", provider: "dropbox" });
