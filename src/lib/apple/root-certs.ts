/**
 * Apple Root CA certificates (DER, base64-embedded) for verifying the x5c
 * certificate chain on App Store Server Notifications V2 and signed transactions.
 * Source: https://www.apple.com/certificateauthority/ (AppleRootCA-G3 / -G2).
 * Embedded (not fs-read) so they ship reliably in the serverless bundle.
 */

const APPLE_ROOT_CA_G3_B64 =
  "";

const APPLE_ROOT_CA_G2_B64 =
  "";

/** Apple Root CA certificates as DER Buffers, for SignedDataVerifier. */
export function appleRootCertificates(): Buffer[] {
  return [
    Buffer.from(APPLE_ROOT_CA_G3_B64, "base64"),
    Buffer.from(APPLE_ROOT_CA_G2_B64, "base64"),
  ];
}

/** Numeric App Store ID for The Memory Palace (from the App Store URL id6766101576). */
export const APP_APPLE_ID = 6766101576;

/** iOS bundle identifier (matches capacitor.config.ts appId). */
export const APP_BUNDLE_ID = "ai.thememorypalace.app";
