...
| **LEG-012** | Privacy/analytics | **Server-side signup-capture naar PostHog loopt buiten de cookie-consent om** (bewuste keuze, 2026-09-05). `user_signed_up` wordt server-side gestuurd bij registratie (`actions.ts`, `auth/callback/route.ts`, via bestaande `captureServer`-helper) + eenmalige backfill van 215 bestaande users. Uitsluitend pseudoniem Supabase-uid + timestamp + signup-methode; geen e-mail/IP/device-data/cookies → geen ePrivacy-opslag/uitlezing, grondslag = gerechtvaardigd belang (first-party accountstatistiek). **Uitbreiding owner-keuze 2026-09-05 (avond): display_name wordt als person-property `name` meegestuurd** (signup-flows + eenmalige backfill) zodat de owner in PostHog/dagmail namen ziet i.p.v. uids → PostHog bevat nu direct identificerende persoonsgegevens (naam), niet enkel pseudoniemen; blijft zonder e-mail/IP/device-data. | W2·I3·B3 | Korte LIA (belangenafweging) vastleggen; PostHog-server-events + naam-property benoemen in privacyverklaring (PostHog staat al als verwerker); opt-out-route bevestigen bij accountverwijdering (person incl. naam in PostHog wissen via delete-person — urgenter nu er namen staan). | ⚖️ licht + ✅ |
